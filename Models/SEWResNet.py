...
def zero_init_blocks(net: nn.Module, connect_f: str):
    for m in net.modules():
        if isinstance(m, Bottleneck):
            nn.init.constant_(m.conv3[1].weight, 0)  # .module
            if connect_f == 'AND':
                nn.init.constant_(m.conv3[1].bias, 1)  # .module
        elif isinstance(m, BasicBlock):
            nn.init.constant_(m.conv2[1].weight, 0)  # .module
            if connect_f == 'AND':
                nn.init.constant_(m.conv2[1].bias, 1)  # .module
...
def _ms_resnet(block, layers, **kwargs):
...
def spiking_resnet18(**kwargs):
    return _sew_resnet(SpikingBasicBlock, [2, 2, 2, 2], **kwargs)
...
def spiking_resnet34(**kwargs):
    return _sew_resnet(SpikingBasicBlock, [3, 4, 6, 3], **kwargs)


def ms_resnet34(**kwargs):
    return _ms_resnet(MSBlock, [3, 4, 6, 3], **kwargs)
